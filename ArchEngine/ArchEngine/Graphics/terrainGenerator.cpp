...
namespace Graphics {
	unsigned TerrainGenerator::genHeightMapTerrain(int terrain_width,
		int terrain_height, const std::string& height_map_path) {
		...
		vertices.resize(terrain_width * terrain_height);
		indices.reserve(terrain_width * terrain_height * 6);
		...
		for (int i = 0; i < terrain_width; i++) {
			for (int j = 0; j < terrain_height; j++) {
				...
				vertices[index].texture_coords.y = (float)j / terrain_height;
			}
		}

		calcNormals(terrain_width, terrain_height, vertices);
	...
	void TerrainGenerator::calcNormals(int terrain_width, int terrain_height,
		...
		// "terrain_height - 1, 0"
		vertices[(terrain_height - 1) * terrain_width].normal = glm::cross(
			vertices[(terrain_height - 2) * terrain_width].position -
				vertices[(terrain_height - 1) * terrain_width].position,
			vertices[(terrain_height - 1) * terrain_width + 1].position -
				vertices[(terrain_height - 1) * terrain_width].position
		);
		// "terrain_width - 1, terrain_height - 1"
		vertices[terrain_height * terrain_width - 1].normal = glm::cross(
			vertices[terrain_height * terrain_width - 2].position -
		...
		for (int i = 1; i < terrain_width - 1; i++) {
			...
			int row_index = (terrain_height - 1) * terrain_width;
			int previous_row_index = (terrain_height - 2) * terrain_width;
